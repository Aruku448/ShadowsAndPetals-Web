(() => {
  ...
  $("[data-page-image]")?.addEventListener("change", async (event) => { const file = event.target.files?.[0]; const page = selectedPage(); if (!file || !page) return; try { const previous = clone(state); page.heroImage = await resizeImage(file); remember(previous); render(); saveState(); showToast("子页头图已替换"); } catch { showToast("无法读取这张图片"); } event.target.value = ""; });
