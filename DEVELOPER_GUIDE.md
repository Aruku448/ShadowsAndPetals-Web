...
2. 子页正文节点使用 `data-page-field-display`，编辑器表单使用 `data-page-field`。两者都应指向 `pages[]` 中的同名字段。正文使用 Markdown 原文保存，运行时由本地 `assets/vendor/marked.umd.js` 解析，并经过 `assets/vendor/purify.min.js` 白名单清理后渲染；不要把渲染后的 HTML 写回配置。正文中的 `h2`、`h3` 会自动生成文章大纲，大纲按钮与正文 CTA 分开工作。
...
6. 子页 `body` 正文编辑上限为 300000 字（30W），编辑器保持至少 280px 高；不要重新加回 1800 字的短限制。
