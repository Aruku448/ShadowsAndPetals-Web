...
- 依赖：`package.json`
- 语法检查：`npm run check`
- 灰度测试：编辑入口（“自定义”按钮）默认隐藏。开发者可通过 URL 参数 `?edit=1`（打开时自动恢复按钮显示），或在控制台执行 `openAshfallEditor()` / `closeAshfallEditor()` 打开/关闭编辑器；后续正式开放时删除 `styles.css` 末尾的 `.edit-button { display: none; }` 即可恢复入口。
